...
regular_editing = 0
detected_times = []  # Store timestamps of detected clips to prevent duplicates
...
def auto_game_montage(*args, progress_callback=None):
    ...
    if ';' in args[0]:
        ...
        if progress_callback:
            progress_callback("Combining your clips...")
        else:
        ...
        if progress_callback:
            progress_callback("Done combining clips.")
        else:
    ...
    if frame_height not in valid_heights or frame_width not in valid_widths:
        if progress_callback:
            progress_callback(f"Your video resolution is {frame_width}x{frame_height}, it needs to be one of these resolutions: 640x480, 1280x720, 1920x1080, 2560x1440, 3840x2160")
        else:
    ...
    cap.set(cv2.CAP_PROP_BUFFERSIZE, 1)  # Reduce buffer size for faster frame reading
    ret, frame = cap.read()
    clips_list = []
    audio_clips_list = []
    global detected_times
    detected_times = []  # Reset detected_times for new video
    which_game = args[4]
    ...
    if which_game == "CSGO":
        ...
        model_path = absolute_paths(r"YOLOmodels/apex_legends.pt")
    else:
    ...
    while ret:  # iterating through the video frames and skipping 1 every iteration
        ...
        frame_count = locals().get('frame_count', 0) + 1
        if frame_count % 10 == 0:  # Update every 10 frames
            if progress_callback:
                progress_callback(f'I\'m watching {str(current_timestamp / 60000)[:6]} minute(s) in your video | Detections: {detections}')
            else:
                sys.stdout.flush()
                sys.stdout.write(
        ...
        def read_screen():
            ...
            global detection_already_found, detected_times
            for template in templates:
                ...
                for pt in zip(*loc[::-1]):
                    ...
                    # Check if current timestamp is too close to recent detections (within 3 seconds)
                    current_time_sec = current_timestamp / 1000
                    if detected_times and current_time_sec - detected_times[-1] < 3:
                        stop_running = True
                    if not stop_running:
                        ...
                        # Overlap checking: adjust start if it overlaps with last clip
                        if clips_list:
                            last_clip_end = clips_list[-1].end
                            if time_clip1 < last_clip_end:
                                time_clip1 = last_clip_end
                                time_clip2 = time_clip1 + (win_status[2] - win_status[1]) / 1000
                        ...
                        detected_times.append(current_timestamp / 1000)
                        try:
                        ...
                        try:
                            ...
                            exception2 = True
                        if not turn_off_effects:
                            target_clip = target_clip.resize(lambda t: 1 + 0.1 * t)  # Reduced from 0.3 to 0.1
                            target_clip = target_clip.fx(vfx.colorx, 1.2)  # Reduced from 1.5 to 1.2
                            target_clip = target_clip.speedx(0.9)  # Increased from 0.6 to 0.9
                            target_audio = target_clip.audio
                            background_sfx = AudioFileClip(absolute_paths("editing_sfx/bass_boosted_fixed.mp3"))
                            background_sfx = background_sfx.audio_fadeout(0.3)  # Increased from 0.2 to 0.3
                            try:
                        ...
                        # Skip multiple frames after successful detection to prevent consecutive detections
                        skip_frames = amount_frames_to_skip * 5
        ...
        def watch_screen():
            global nothing_or_something, start, end, exception1, exception2, detections, detected_times
            nonlocal frame
            if frame is None:
                return
            ...
            for result in results.boxes.data.tolist():
                ...
                if (score > threshold) and ('kill' in class_name or 'knock' in class_name or 'apex' in class_name or class_name == '0'):
    ...
    try:
        ...
        # Resolution check: prevent upscaling beyond 1080p if input is lower
        output_resolution = f"{frame_width}x{frame_height}"
        if frame_height > 1080:
            if progress_callback:
                progress_callback(f"Quality check: Input resolution {output_resolution}, FPS: {fps}")
            else:
                print(f"\nProgram: Quality check: Input resolution {output_resolution}, FPS: {fps}")
        
        if progress_callback:
            progress_callback(f"Editing your video now. ETA: {editing_eta} minute(s) | Resolution: {output_resolution} | FPS: {fps}")
        else:
            print(f"\nProgram: Editing your video now. ETA: {editing_eta} minute(s) | Resolution: {output_resolution} | FPS: {fps}")
        final_clip.write_videofile(f"{args[1]}/{output_filename}_output.mp4", codec='libx264', audio_codec='aac',
                                   preset='slow', bitrate='12000k', threads=4, ffmpeg_params=["-crf", "18"], logger=None)
        if progress_callback:
            progress_callback(f"All done! The edited video can be found here: {args[1]}/{output_filename}_output.mp4")
        else:
            print(f"Program: All done! The edited video can be found here: {args[1]}/{output_filename}_output.mp4")
    except Exception as e:
        if progress_callback:
            progress_callback(f"Error: {e}\nNo detections found of the game {which_game} in the selected video.")
        else:
            print(e)
